...
function orderPage(req, res) {
    ...
    if(req.session.cart != null) {
        for(id in req.session.cart) {
            total += req.session.cart[id].price * req.session.cart[id].qty;
        }
    }

    
    res.render("Page/order(old)", {title: "ATN-Shop ORDER page", 
        content: xcontent , itemlist: req.session.cart,  // Object.values(itemlist)
        configHeader: configHeader  , currpage: "Order" , total: total });
